...
class Colors:
    ...
    def get_all_colors(cls):
        """Returns dict of all available colors"""
        return {
            'black': cls.BLACK,
            'red': cls.RED,
            'green': cls.GREEN,
...
def load_config():
    ...
    default_config = {
        ...
        'device_actions': {},
        'device_colors': {},  # NEU: Device-Farben
        'vendor_colors': {},  # NEU: Vendor-Farben
        'volume': 100,
        'profiles': {
            'default': {
                'disconnect_sound': None,
...
def get_device_color(device_id, vendor_id, config):
    """Get color for device with priority: device > vendor > default(white)"""
    if device_id in config.get('device_colors', {}):
        color_name = config['device_colors'][device_id]
        return Colors.get_color(color_name)
    elif vendor_id and vendor_id in config.get('vendor_colors', {}):
        color_name = config['vendor_colors'][vendor_id]
        return Colors.get_color(color_name)
    return Colors.WHITE
...
def monitor_usb(hide_connects=False, hide_disconnects=False, hide_default=False, hide_devices=False, show_all_duplicates=False):
    ...
    def handle_device_event(action, device):
        ...
        if device_id in config.get('blacklist', []):
            if debug_mode:
                print(f"DEBUG: Device {device_id} is blacklisted")
            return
...
def set_color(device_name, color_name):
    """Set color for device or vendor"""
    config = load_config()
...
def show_stats():
    ...
    for event in history:
        ...
        if device not in stats:
            stats[device] = {'connects': 0, 'disconnects': 0, 'vendor': vendor}
...
def remove_config(config_type, device_name):
    ...
    if config_type == 'sound':
        ...
        if device_name in config.get('device_actions', {}):
            ...
            return
    elif config_type == 'color' or config_type == 'colour':
        if device_name.startswith('vendor:'):
            vendor_id = device_name.split(':', 1)[1]
    ...
    versions = [
        {
            ...
            'changes': [
                'Added color support for devices and vendors',
                'Colorful output for history, stats, and monitoring',
                'Fixed autostart issues with systemd service',
                'Better autostart wrapper with delay',
                'Service status check after installation',
                'Tab completion support for colors (OS-dependent)',
                'Show all available colors with examples'
            ]
        },
        {
            'version': '3.1',
        ...
        },
        {
            ...
            'changes': [
                'Added vendor-specific sounds',
                'Added action scripts on device connect',
                'Added device history tracking',
                'Added device statistics',
...
def main():
    ...
    if args.man:
        ...
        print("    kg colour [DEVICE|vendor:XXXX] COLOR")
        print("    kg remove [sound|action|colour] [DEVICE|vendor:XXXX|!]")
        print("    kg colours")
        ...
        print("            kg colour vendor:1532 gray")
        print("            kg colour 8BitDo_IDLE white")
        print("")
        print("    " + colorize("colours", Colors.BRIGHT_GREEN))
        print("        Show all available colors with examples")
        print("")
        print("    For all other commands, see kg --help")
        print("\nAUTHOR")
